...
// 좌표가 없는 일정·숙소는 null을 유지한다. Number(null)은 0이므로 값 검사만으로는 (0,0)으로 저장된다.
function coord(value) { if (value === null || value === undefined || value === '') return null; const number = Number(value); return Number.isFinite(number) ? number : null; }
...
function validateTrip(raw) {
  ...
  const trip = {
    ...
    checklist:Array.isArray(raw.checklist)?raw.checklist.slice(0,200).map((x,index)=>({id:clean(x.id,100),title:clean(x.title,100),scope:x.scope==='shared'?'shared':'personal',assigneeMemberId:clean(x.assigneeMemberId,100),completed:Boolean(x.completed),sortOrder:Number.isSafeInteger(Number(x.sortOrder))?Number(x.sortOrder):index,important:Boolean(x.important)})).filter(x=>x.id&&x.title):[],expenseSettings:{baseCurrency:EXPENSE_CURRENCIES.has(raw.expenseSettings?.baseCurrency)?raw.expenseSettings.baseCurrency:'KRW',budgetMinor:Number.isSafeInteger(raw.expenseSettings?.budgetMinor)&&raw.expenseSettings.budgetMinor>=0?raw.expenseSettings.budgetMinor:null,settledAt:clean(raw.expenseSettings?.settledAt,40),settlementFingerprint:clean(raw.expenseSettings?.settlementFingerprint,20000)},
  ...
  trip.items = trip.items.map(x => ({
    ...
    place:clean(x.place,500), mapUrl:clean(x.mapUrl,1500), memo:clean(x.memo,4000), move:clean(x.move,30), alarm:clean(x.alarm,5), reminderMinutes:[10,30,60].includes(Number(x.reminderMinutes))?Number(x.reminderMinutes):0,reservationNumber:clean(x.reservationNumber,100), provider:clean(x.provider,200),
    ...
    lat:coord(x.lat), lng:coord(x.lng)
  }));
...
function childStatements(env, trip, memberId) {
  ...
  for(const x of trip.items)q.push(env.DB.prepare(`INSERT INTO items (id,trip_id,day,time,end_time,preparation_minutes,fixed_schedule,move_minutes,reminder_minutes,category,name,place,map_url,memo,move,alarm,reservation_number,provider,lat,lng,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(x.id,trip.id,x.day,x.time,x.endTime,x.preparationMinutes,x.fixed?1:0,x.moveMinutes,x.reminderMinutes,x.cat,x.name,x.place,x.mapUrl,x.memo,x.move,x.alarm,x.reservationNumber,x.provider,x.lat,x.lng,stamp,stamp));
...
function entityLabel(type,value){return clean(type==='file'?value.name:type==='flight'?[value.airline,value.flightNumber].filter(Boolean).join(' · '):type==='lodging'?value.name:type==='expense'?(value.title||value.category):value.name,180)||'이름 없는 항목'}
...
function activityValue(value){if(value===null||value===undefined)return null;if(typeof value==='string')return value.slice(0,300);if(typeof value==='number'||typeof value==='boolean')return value;if(Array.isArray(value))return value.slice(0,10).map(activityValue);return JSON.stringify(value).slice(0,300)}
function activityValues(before,after,keys){return Object.fromEntries(keys.map(key=>[key,{before:activityValue(before?.[key]),after:activityValue(after?.[key])}]))}
function collaborationChanges(before,after){const changes=[],tripKeys=changedKeys(before,after);if(tripKeys.length)changes.push({action:'updated',type:'trip',id:after.id,label:after.title,keys:tripKeys,values:activityValues(before,after,tripKeys)});for(const[type,group]of ENTITY_GROUPS){const oldMap=new Map((before[group]||[]).map(x=>[x.id,x])),newMap=new Map((after[group]||[]).map(x=>[x.id,x]));for(const[id,value]of oldMap)if(!newMap.has(id))changes.push({action:'deleted',type,id,label:entityLabel(type,value),before:value,files:(before.files||[]).filter(f=>f.entityType===type&&f.entityId===id),keys:[]});for(const[id,value]of newMap)if(!oldMap.has(id))changes.push({action:'created',type,id,label:entityLabel(type,value),keys:[]});else{const previous=oldMap.get(id),keys=changedKeys(previous,value);if(keys.length)changes.push({action:'updated',type,id,label:entityLabel(type,value),keys,values:activityValues(previous,value,keys)})}}const deletedParents=new Set(changes.filter(x=>x.action==='deleted').map(x=>`${x.type}|${x.id}`)),oldFiles=new Map((before.files||[]).map(x=>[x.id,x])),newFiles=new Map((after.files||[]).map(x=>[x.id,x]));for(const[id,value]of oldFiles)if(!newFiles.has(id)&&!deletedParents.has(`${value.entityType}|${value.entityId}`))changes.push({action:'deleted',type:'file',id,label:entityLabel('file',value),before:value,keys:[]});for(const[id,value]of newFiles)if(!oldFiles.has(id))changes.push({action:'created',type:'file',id,label:entityLabel('file',value),keys:[]});else{const previous=oldFiles.get(id),keys=changedKeys(previous,value);if(keys.length)changes.push({action:'updated',type:'file',id,label:entityLabel('file',value),keys,values:activityValues(previous,value,keys)})}return changes}
function activityStatements(env,tripId,memberId,changes,stamp){const q=[];for(const change of changes){q.push(env.DB.prepare(`INSERT INTO trip_activity (id,trip_id,member_id,action,entity_type,entity_id,label,detail_json,created_at) VALUES (?,?,?,?,?,?,?,?,?)`).bind(id('act'),tripId,memberId,change.action,change.type,change.id,change.label,JSON.stringify({fields:change.keys,values:change.values||{}}),stamp));if(change.action==='deleted'&&change.type!=='trip')q.push(env.DB.prepare(`INSERT INTO trip_trash (id,trip_id,entity_type,entity_id,label,snapshot_json,deleted_by_member_id,deleted_at) VALUES (?,?,?,?,?,?,?,?) ON CONFLICT(trip_id,entity_type,entity_id) WHERE restored_at IS NULL DO UPDATE SET label=excluded.label,snapshot_json=excluded.snapshot_json,deleted_by_member_id=excluded.deleted_by_member_id,deleted_at=excluded.deleted_at`).bind(id('trash'),tripId,change.type,change.id,change.label,JSON.stringify({entity:change.before,files:change.files}),memberId,stamp))}return q}
function accessActivityStatement(env,tripId,memberId,{category,action,semanticAction=action,id:entityId,label,fields=[],values={}},stamp){return env.DB.prepare(`INSERT INTO trip_activity (id,trip_id,member_id,action,entity_type,entity_id,label,detail_json,created_at) VALUES (?,?,?,?,?,?,?,?,?)`).bind(id('act'),tripId,memberId,action,'trip',entityId,label,JSON.stringify({category,semanticAction,fields,values}),stamp)}
...
async function updateTrip(request,env,tripId,member){
  ...
  const assertion=id('assert'),stamp=now(),changes=collaborationChanges(before,trip),statements=[
    env.DB.prepare(`UPDATE trips SET title=?,start_date=?,end_date=?,note=?,cities_json=?,checklist_json=?,hero_file_id=?,base_currency=?,budget_minor=?,settled_at=?,settlement_fingerprint=?,revision=revision+1,updated_at=? WHERE id=? AND revision=?`).bind(trip.title,trip.start,trip.end,trip.note,JSON.stringify(trip.cities),JSON.stringify(trip.checklist),trip.heroFileId||null,trip.expenseSettings.baseCurrency,trip.expenseSettings.budgetMinor,trip.expenseSettings.settledAt||null,trip.expenseSettings.settlementFingerprint||null,stamp,tripId,base),
...
async function health(env){
  let db='error';
  try{const probe=await env.DB.prepare('SELECT 1 AS ok').first();db=probe?.ok===1?'ok':'error'}catch{db='error'}
  return json({ok:db==='ok',db,time:new Date().toISOString()},db==='ok'?200:503);
}
