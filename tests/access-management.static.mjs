...
assert.equal((source.match(/function recoveryResult/g) || []).length, 0, '신규 복구 코드 결과 화면 제거');
assert.doesNotMatch(source, /issueRecoveryKey|recoveryResult|data-issue-recovery/, '신규 복구 코드 발급 UI 제거');
...
assert.match(settingsSource, /여행 관리[\s\S]*dataSafety[\s\S]*myShare[\s\S]*tripShare[\s\S]*collaborationLog[\s\S]*앱 기능[\s\S]*notificationSettings[\s\S]*preparation[\s\S]*allIcs[\s\S]*고급 데이터 관리[\s\S]*exportJson[\s\S]*데이터 내보내기[\s\S]*importJson[\s\S]*데이터 가져오기[\s\S]*앱 안내[\s\S]*초기화/, '설정 항목을 사용 목적과 우선순위에 따라 그룹화');
...
assert.match(source, /async function resetProtectionSheet/, '전체 초기화 전에 동기화와 원본 상태 확인');
assert.match(source, /원할 때 만드는 독립 사본입니다\.[\s\S]*자동으로 생성되거나 다운로드되지 않습니다\./, 'JSON 내보내기는 수동 독립 데이터 사본으로 안내');
assert.doesNotMatch(source, /백업 필요|백업 후 변경사항|최신 상태로 백업되어 있어요/, '일반 안전 화면에서 JSON 백업 상태 제거');
assert.match(source, /YeogiroStore\.completeBackup\(backup\.state,backup\.exportedAt\)/, '사용자가 파일 생성을 실행한 뒤 내보내기 기준점 갱신');
assert.match(source, /resetProtectionSheet[\s\S]*data-reset-backup>데이터 내보내기[\s\S]*데이터를 내보내지 않고 전체 초기화/, '전체 초기화에서 선택적 데이터 내보내기 제공');
assert.doesNotMatch(source.match(/async function dataSafetySheet\(\)[\s\S]*?const APP_BUILD/)?.[0] || '', /JSON|복구 코드|backup|recovery/i, '데이터 보관 상태에서 JSON·복구 상태 제거');
...
assert.match(settingsSource, /id="exportJson"[\s\S]*데이터 내보내기[\s\S]*id="importJson"[\s\S]*데이터 가져오기/, 'JSON 기능은 설정의 고급 데이터 관리에서 유지');
...
assert.match(source, /async function dataSafetySheet/, '동기화·원본·저장공간·보관 설정 화면 제공');
...
assert.match(source, /register\('\/sw\.js\?v=\d+',[\s\S]*updateViaCache:'none'/, 'versioned service worker is registered without the HTTP cache');
