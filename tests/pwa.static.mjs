...
assert.match(source, /yeogiro-app-v67/, 'app shell cache version was advanced');
assert.match(source, /'\/sync-ui\.js\?v=67'/, 'conflict UI uses a versioned app-shell URL');
assert.match(source, /'\/travel-logic\.js\?v=67'/, 'travel state logic remains available offline');
assert.match(source, /'\/data-integrity\.js\?v=67'/, 'data integrity checks remain available offline');
assert.match(source, /'\/expense-logic\.js\?v=67'/, 'expense calculations remain available offline');
assert.match(source, /'\/weather-logic\.js\?v=67'/, 'cached weather presentation logic remains available offline');
assert.match(source, /'\/pwa-update\.js\?v=67'/, 'update gating logic remains available offline');
