...
assert.match(source, /yeogiro-app-v54/, 'app shell cache version was advanced');
assert.match(source, /'\/sync-ui\.js\?v=54'/, 'conflict UI uses a versioned app-shell URL');
assert.match(source, /'\/travel-logic\.js\?v=54'/, 'travel state logic remains available offline');
assert.match(source, /'\/data-integrity\.js\?v=54'/, 'data integrity checks remain available offline');
assert.match(source, /'\/expense-logic\.js\?v=54'/, 'expense calculations remain available offline');
assert.match(source, /'\/weather-logic\.js\?v=54'/, 'cached weather presentation logic remains available offline');
assert.match(source, /'\/pwa-update\.js\?v=54'/, 'update gating logic remains available offline');
