...
assert.match(source, /yeogiro-app-v64/, 'app shell cache version was advanced');
assert.match(source, /'\/sync-ui\.js\?v=64'/, 'conflict UI uses a versioned app-shell URL');
assert.match(source, /'\/travel-logic\.js\?v=64'/, 'travel state logic remains available offline');
assert.match(source, /'\/data-integrity\.js\?v=64'/, 'data integrity checks remain available offline');
assert.match(source, /'\/expense-logic\.js\?v=64'/, 'expense calculations remain available offline');
assert.match(source, /'\/weather-logic\.js\?v=64'/, 'cached weather presentation logic remains available offline');
assert.match(source, /'\/pwa-update\.js\?v=64'/, 'update gating logic remains available offline');
