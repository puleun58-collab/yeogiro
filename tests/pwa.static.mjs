...
assert.match(source, /yeogiro-app-v40/, 'app shell cache version was advanced');
assert.match(source, /'\/sync-ui\.js\?v=40'/, 'conflict UI uses a versioned app-shell URL');
assert.match(source, /'\/travel-logic\.js\?v=40'/, 'travel state logic remains available offline');
assert.match(source, /'\/data-integrity\.js\?v=40'/, 'data integrity checks remain available offline');
