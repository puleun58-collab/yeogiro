...
assert.match(source, /yeogiro-app-v75/, 'app shell cache version was advanced');
assert.match(source, /'\/sync-ui\.js\?v=75'/, 'conflict UI uses a versioned app-shell URL');
assert.match(source, /'\/travel-logic\.js\?v=75'/, 'travel state logic remains available offline');
assert.match(source, /'\/data-integrity\.js\?v=75'/, 'data integrity checks remain available offline');
assert.match(source, /'\/expense-logic\.js\?v=75'/, 'expense calculations remain available offline');
assert.match(source, /'\/weather-logic\.js\?v=75'/, 'cached weather presentation logic remains available offline');
assert.match(source, /'\/pwa-update\.js\?v=75'/, 'update gating logic remains available offline');
assert.match(source, /'\/trip-recap-logic\.js\?v=75'/, 'trip recap logic remains available offline');
