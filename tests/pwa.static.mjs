...
assert.match(source, /yeogiro-app-v71/, 'app shell cache version was advanced');
assert.match(source, /'\/sync-ui\.js\?v=71'/, 'conflict UI uses a versioned app-shell URL');
assert.match(source, /'\/travel-logic\.js\?v=71'/, 'travel state logic remains available offline');
assert.match(source, /'\/data-integrity\.js\?v=71'/, 'data integrity checks remain available offline');
assert.match(source, /'\/expense-logic\.js\?v=71'/, 'expense calculations remain available offline');
assert.match(source, /'\/weather-logic\.js\?v=71'/, 'cached weather presentation logic remains available offline');
assert.match(source, /'\/pwa-update\.js\?v=71'/, 'update gating logic remains available offline');
