...
assert.match(source, /yeogiro-app-v48/, 'app shell cache version was advanced');
assert.match(source, /'\/sync-ui\.js\?v=48'/, 'conflict UI uses a versioned app-shell URL');
assert.match(source, /'\/travel-logic\.js\?v=48'/, 'travel state logic remains available offline');
assert.match(source, /'\/data-integrity\.js\?v=48'/, 'data integrity checks remain available offline');
assert.match(source, /'\/pwa-update\.js\?v=48'/, 'update gating logic remains available offline');
