...
assert.match(source, /yeogiro-app-v68/, 'app shell cache version was advanced');
assert.match(source, /'\/sync-ui\.js\?v=68'/, 'conflict UI uses a versioned app-shell URL');
assert.match(source, /'\/travel-logic\.js\?v=68'/, 'travel state logic remains available offline');
assert.match(source, /'\/data-integrity\.js\?v=68'/, 'data integrity checks remain available offline');
assert.match(source, /'\/expense-logic\.js\?v=68'/, 'expense calculations remain available offline');
assert.match(source, /'\/weather-logic\.js\?v=68'/, 'cached weather presentation logic remains available offline');
assert.match(source, /'\/pwa-update\.js\?v=68'/, 'update gating logic remains available offline');
