...
assert.match(source, /yeogiro-app-v47/, 'app shell cache version was advanced');
assert.match(source, /'\/sync-ui\.js\?v=47'/, 'conflict UI uses a versioned app-shell URL');
assert.match(source, /'\/travel-logic\.js\?v=47'/, 'travel state logic remains available offline');
assert.match(source, /'\/data-integrity\.js\?v=47'/, 'data integrity checks remain available offline');
assert.match(source, /'\/pwa-update\.js\?v=47'/, 'update gating logic remains available offline');
...
console.log('12 PWA cache checks passed');
