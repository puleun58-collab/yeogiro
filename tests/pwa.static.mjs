...
assert.match(source, /yeogiro-app-v27/, 'app shell cache version was advanced');
assert.match(source, /'\/sync-ui\.js\?v=27'/, 'conflict UI uses a versioned app-shell URL');
assert.match(source, /'\/travel-logic\.js\?v=27'/, 'travel state logic remains available offline');
assert.match(source, /'\/data-integrity\.js\?v=27'/, 'data integrity checks remain available offline');
