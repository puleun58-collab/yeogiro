...
assert.match(source, /yeogiro-app-v81/, 'app shell cache version was advanced');
assert.match(source, /'\/sync-ui\.js\?v=81'/, 'conflict UI uses a versioned app-shell URL');
assert.match(source, /'\/travel-logic\.js\?v=81'/, 'travel state logic remains available offline');
assert.match(source, /'\/data-integrity\.js\?v=81'/, 'data integrity checks remain available offline');
assert.match(source, /'\/diagnostics\.js\?v=81'/, 'diagnostics and error log remain available offline');
assert.match(source, /'\/expense-logic\.js\?v=81'/, 'expense calculations remain available offline');
assert.match(source, /'\/weather-logic\.js\?v=81'/, 'cached weather presentation logic remains available offline');
assert.match(source, /'\/pwa-update\.js\?v=81'/, 'update gating logic remains available offline');
assert.match(source, /'\/trip-recap-logic\.js\?v=81'/, 'trip recap logic remains available offline');
