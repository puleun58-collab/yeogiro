...
assert.match(source, /yeogiro-app-v45/, 'app shell cache version was advanced');
assert.match(source, /'\/sync-ui\.js\?v=45'/, 'conflict UI uses a versioned app-shell URL');
assert.match(source, /'\/travel-logic\.js\?v=45'/, 'travel state logic remains available offline');
assert.match(source, /'\/data-integrity\.js\?v=45'/, 'data integrity checks remain available offline');
assert.match(source, /'\/pwa-update\.js\?v=45'/, 'update gating logic remains available offline');
