...
assert.match(source, /yeogiro-app-v46/, 'app shell cache version was advanced');
assert.match(source, /'\/sync-ui\.js\?v=46'/, 'conflict UI uses a versioned app-shell URL');
assert.match(source, /'\/travel-logic\.js\?v=46'/, 'travel state logic remains available offline');
assert.match(source, /'\/data-integrity\.js\?v=46'/, 'data integrity checks remain available offline');
assert.match(source, /'\/pwa-update\.js\?v=46'/, 'update gating logic remains available offline');
