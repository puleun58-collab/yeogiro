...
assert.match(source, /yeogiro-app-v74/, 'app shell cache version was advanced');
assert.match(source, /'\/sync-ui\.js\?v=74'/, 'conflict UI uses a versioned app-shell URL');
assert.match(source, /'\/travel-logic\.js\?v=74'/, 'travel state logic remains available offline');
assert.match(source, /'\/data-integrity\.js\?v=74'/, 'data integrity checks remain available offline');
assert.match(source, /'\/expense-logic\.js\?v=74'/, 'expense calculations remain available offline');
assert.match(source, /'\/weather-logic\.js\?v=74'/, 'cached weather presentation logic remains available offline');
assert.match(source, /'\/pwa-update\.js\?v=74'/, 'update gating logic remains available offline');
assert.match(source, /'\/trip-recap-logic\.js\?v=74'/, 'trip recap logic remains available offline');
