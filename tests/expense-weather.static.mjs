...
assert.match(source, /dailyAdvice\(day,upcomingHours\)\[0\]\?\.text/, '규칙형 날씨 안내 중 가장 중요한 한 건만 표시');
...
// PWA 자산
assert.match(serviceWorker, /'\/expense-logic\.js\?v=77'/, '경비 로직을 오프라인 캐시에 포함');
assert.match(serviceWorker, /'\/weather-logic\.js\?v=77'/, '날씨 표시 로직을 오프라인 캐시에 포함');
