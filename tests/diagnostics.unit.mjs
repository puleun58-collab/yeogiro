...
const report = Diag.report({
  ...
  serviceWorker: '활성 · yeogiro-app-v80',
...
assert.match(report, /Service Worker: 활성 · yeogiro-app-v80/, '복사 정보에 SW 상태 포함'); check();
...
assert.match(html, /<script src="\/diagnostics\.js\?v=80"><\/script>/, '진단 모듈 로드'); check();
