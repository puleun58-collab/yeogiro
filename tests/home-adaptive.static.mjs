...
assert.doesNotMatch(before, /D-\$\{daysLeft\}|여행 전 · D-/, '여행 준비 카드에서 D-Day 중복 제거');
assert.match(source, /\$\('#dday'\)\.textContent=YeogiroTravel\.tripStatusLabel\(t,todayKey\)/, '상단 D-Day가 공통 상태 helper를 사용');
assert.match(source, /\$\('#dday'\)\.dataset\.phase=phase/, '상단 D-Day에 여행 단계 상태 제공');
...
assert.doesNotMatch(before, /fmt\(t\.start\)|homeCityLine|home-facts|첫 일정 ·|항공편 ·|숙소 ·/, '준비 카드에서 여행 기간과 일정 기능 정보를 반복하지 않음');
...
assert.match(before, /출발 전 확인 \$\{stats\.required\.length\}건/, '필수 확인 항목 수를 같은 점검 결과로 표시');
assert.doesNotMatch(before, /확인 \$\{stats\.required\.length\}건<span aria-hidden="true"> !/, '확인 건수 뒤 느낌표 제거');
assert.match(before, /class="home-state \$\{stats\.required\.length\?'warn':'ok'\}" data-preparation/, '출발 전 확인 행 전체를 준비 시트 바로가기로 제공');
...
assert.doesNotMatch(before, /data-before-trip=|data-after-trip=/, '준비 카드에서 일정과 예약 CTA 제거');
