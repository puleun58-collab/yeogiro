...
- 앱 아이콘과 제목 아래 연한 neutral 영역에 `Chrome`과 `Safari` 섹션을 divider로 구분해 나란히 보여줍니다.
- 각 브라우저는 핵심 2단계만 표시합니다. Chrome은 `더보기 메뉴` → `홈 화면에 추가`, Safari는 `공유` → `홈 화면에 추가`이며 단계마다 시스템 아이콘을 함께 보여줍니다.
- 각 섹션 오른쪽 `상세가이드 보기`를 누르면 같은 모달 안에서 3단계 상세 안내와 예외 상황(도구 막대 표시, iOS Chrome에서 Safari로 열기, Samsung Internet 경로)을 확인할 수 있습니다.
