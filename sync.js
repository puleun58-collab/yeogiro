...
async function flush(){
  if(syncing||!navigator.onLine||!stateRef)return;syncing=true;setStatus('동기화 중');
...
async function persist(state){
  stateRef=state;await cacheState();const live=new Set(state.trips.map(x=>x.id)),referenced=new Set();for(const trip of state.trips){for(const item of trip.items||[])for(const doc of item.userDocs||[])if(doc.id)referenced.add(doc.id);for(const lodging of trip.lodgings||[])for(const doc of lodging.userDocs||[])if(doc.id)referenced.add(doc.id);if(trip.heroFileId)referenced.add(trip.heroFileId);await put('outbox',{tripId:trip.id,updatedAt:Date.now()})}
