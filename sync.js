...
const status={online:navigator.onLine,lastSync:'',pending:0,message:'준비 중',phase:'idle',conflict:null,roleByTrip:{}};
...
async function pushTrip(trip){
  ...
  catch(error){if(error.status===409&&error.body?.trip){status.conflict={local:structuredClone(trip),remote:error.body.trip,tripId:trip.id};setStatus('동기화 확인 필요','conflict');window.dispatchEvent(new CustomEvent('yeogiro:conflict',{detail:status.conflict}));throw error}if(error.status===401){await del('sessions',trip.id);status.roleByTrip[trip.id]='';}throw error}
}
async function flush(){
  ...
  try{const queued=await all('outbox');status.pending=queued.length;for(const job of queued){const local=stateRef.trips.find(x=>x.id===job.tripId);if(!local){await del('outbox',job.tripId);continue}const remote=await pushTrip(local),merged=mergeRemote(remote,local);const index=stateRef.trips.findIndex(x=>x.id===job.tripId);if(index>=0)stateRef.trips[index]=merged;await del('outbox',job.tripId);status.pending=Math.max(0,status.pending-1);emit()}status.pending=0;status.lastSync=new Date().toISOString();await put('meta',status.lastSync,'last-sync');await cacheState();setStatus('모든 변경사항이 저장되었습니다.','idle');if(remoteHandler)remoteHandler(stateRef)}catch(error){if(error.status!==409)setStatus('변경사항이 이 기기에 안전하게 저장되어 있습니다.',navigator.onLine?'pending':'offline')}finally{syncing=false}}
...
async function bootstrap(legacy){
  ...
  for(const savedSession of await all('sessions'))status.roleByTrip[savedSession.tripId]=savedSession.role;
  ...
  status.pending=(await all('outbox')).length;status.phase=navigator.onLine?(status.pending?'pending':'idle'):'offline';emit();setTimeout(flush,50);setTimeout(pullAll,600);return stateRef
}
async function persist(state){
  ...
  status.pending=(await all('outbox')).length;setStatus(navigator.onLine?'변경사항이 동기화를 기다리고 있습니다.':'변경사항이 이 기기에 안전하게 저장되어 있습니다.',navigator.onLine?'pending':'offline');clearTimeout(syncTimer);syncTimer=setTimeout(flush,450)
}
...
async function share(role='editor',singleUse=true){const t=stateRef.trips.find(x=>x.id===stateRef.activeId),s=await session(t.id);if(!s)throw new Error('먼저 여행 동기화를 완료해 주세요.');const result=await request(`/api/trips/${encodeURIComponent(t.id)}/invites`,{method:'POST',body:JSON.stringify({role,expiresInDays:7,singleUse})},s.token);return `${location.origin}${location.pathname}?invite=${encodeURIComponent(result.token)}`}
...
async function useRemote(tripId,remote){const index=stateRef.trips.findIndex(x=>x.id===tripId);if(index>=0)stateRef.trips[index]=mergeRemote(remote,stateRef.trips[index]);const s=await session(tripId);if(s){s.revision=remote.revision;await put('sessions',s)}await del('outbox',tripId);status.conflict=null;status.pending=(await all('outbox')).length;await cacheState();setStatus('서버의 최신 내용으로 동기화되었습니다.','idle');if(remoteHandler)remoteHandler(stateRef)}
async function reapplyLocal(tripId,local,remote){const index=stateRef.trips.findIndex(x=>x.id===tripId);if(index<0)return;const preserved=structuredClone(local);preserved.revision=remote.revision;stateRef.trips[index]=preserved;const s=await session(tripId);if(s){s.revision=remote.revision;await put('sessions',s)}await put('outbox',{tripId,updatedAt:Date.now()});status.conflict=null;status.pending=(await all('outbox')).length;await cacheState();setStatus('내 변경사항을 다시 적용하는 중입니다.','pending');if(remoteHandler)remoteHandler(stateRef);await flush()}
...
window.YeogiroStore={bootstrap,persist,addFiles,fileUrl,fileBlob,hasFile,setHero,ensureHero,heroUrl,exportBackup,importBackup,share,access,revoke,onRemote,useRemote,reapplyLocal,pull:pullAll,flush,status:()=>({...status}),device:()=>deviceId};
})();
