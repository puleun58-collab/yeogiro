...
async function bootstrap(legacy){
  ...
  setTimeout(()=>refreshAccount().then(()=>{if(status.pendingInvite&&!status.account)setStatus('초대 여행에 참여하려면 로그인해 주세요.','idle')}).catch(()=>{}),20);setTimeout(flush,50);setTimeout(pullAll,600);return stateRef
}
...
function stableBackupValue(value){
  ...
  return`{${Object.keys(value).filter(key=>!BACKUP_IGNORED_KEYS.has(key)).sort().map(key=>`${JSON.stringify(key)}:${stableBackupValue(value[key])}`).join(',')}}`;
}
...
async function dataSafety(trip){const s=await session(trip.id),queued=(await all('outbox')).filter(x=>x.tripId===trip.id),files=await auditFiles(trip);let recoveryConfigured=null,storage={used:0,quota:0,supported:false,persisted:null,canPersist:Boolean(navigator.storage?.persist)};if(s?.role==='owner')try{recoveryConfigured=Boolean((await access()).recoveryConfigured)}catch{}try{const estimate=await navigator.storage?.estimate?.();if(estimate)storage={...storage,used:Number(estimate.usage)||0,quota:Number(estimate.quota)||0,supported:true}}catch{}try{const persisted=await navigator.storage?.persisted?.();if(typeof persisted==='boolean')storage.persisted=persisted}catch{}const backup=await backupStatus(stateRef);return{connected:Boolean(s),role:s?.role||'',pending:queued.length,conflict:status.conflict?.tripId===trip.id,lastSync:status.lastSync||'',lastBackup:backup.lastBackup,backupChanges:backup.changes,recoveryConfigured,storage,files:{total:files.length,local:files.filter(x=>x.local).length,localOnly:files.filter(x=>x.local&&x.deviceId===deviceId).length,missing:files.filter(x=>!x.local).length,mismatch:files.filter(x=>x.mismatch).length,bytes:files.reduce((sum,x)=>sum+x.actualSize,0),hero:files.filter(x=>x.kind==='trip').length,issues:files.filter(x=>!x.local||x.mismatch)}}}
...
async function issueRecoveryKey(){const t=stateRef.trips.find(x=>x.id===stateRef.activeId),s=await session(t.id);if(!s)throw new Error('동기화 정보가 없습니다.');return request(`/api/trips/${encodeURIComponent(t.id)}/recovery-key`,{method:'POST',body:'{}'},s.token)}
